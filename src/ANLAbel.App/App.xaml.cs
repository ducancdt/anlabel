    ...
    private static void ApplyLicenseDisplay(MainWindow window, TrialCheckResult result)
    {
        if (result.IsActivated)
        {
            if (result.ActivationExpiresUtc is { } expiresUtc)
            {
                ...
                window.Title = $"ANLAbel - Label Designer v0.266 — Đã kích hoạt, còn {daysLeft} ngày (hết hạn {expiresUtc.ToLocalTime():dd/MM/yyyy})";
                window.BuildChannelText.Text = $"LICENSED · còn {daysLeft} ngày · v0.266";
            }
            else
            {
            window.Title = "ANLAbel - Label Designer v0.266 — Đã kích hoạt (vĩnh viễn)";
            window.BuildChannelText.Text = "LICENSED · Vĩnh viễn · v0.266";
            }
        }
        else
        {
            var days = Math.Max(1, (int)Math.Ceiling(result.Remaining.TotalDays));
            window.Title = $"ANLAbel - Label Designer v0.266 — Dùng thử còn {days} ngày";
            window.BuildChannelText.Text = "TRIAL 7 NGÀY · v0.266";
        }
    }
