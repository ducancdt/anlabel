...
| Data Workspace authoring/diagnostics | Same Figma file, Data shell `8:2`/`9:2`, empty/current/settings/checks `9:3`, `9:16`, `9:27`, `9:35`; no transform editor states | Define the first transform-authoring task, then approve shell reuse or name a state-specific reference before adding controls. See [`R4_DATA_WORKSPACE_UI_HANDOFF.md`](../R4_DATA_WORKSPACE_UI_HANDOFF.md) and its reviewable [`R4_DATA_WORKSPACE_UI_SPEC.md`](../R4_DATA_WORKSPACE_UI_SPEC.md). |
| Database Manager | No dedicated frame in panels Page `0:1`; current WPF `DatabaseManagerWindow` has the planned list/detail, Test/Preview/Use/Remove/Cleanup states | Follow [`DATABASE_MANAGER_UI_HANDOFF.md`](../DATABASE_MANAGER_UI_HANDOFF.md) and [`DATABASE_MANAGER_UI_SPEC.md`](../DATABASE_MANAGER_UI_SPEC.md): approve current WPF information architecture or name a state-specific reference, then attach runtime click-through evidence. |
| Barcode authoring Properties | Read-only Page `0:1` scan on 2026-08-13 finds only ribbon text layer `1:8`; no barcode-properties/check-digit/HRI state-specific frame. Current WPF card is [`MainWindow.xaml#L1878`](../../src/ANLAbel.App/MainWindow.xaml#L1878) through the barcode validation/readout controls | P1/P2 software slices are closed. For P3, use `18:69`/`13:2` as interim selected-Properties language only; the owner must approve that reuse or name a state-specific node before changing the panel. See [`P3_BARCODE_CHECK_DIGIT_UI_HANDOFF.md`](../P3_BARCODE_CHECK_DIGIT_UI_HANDOFF.md) and its reviewable [`P3_BARCODE_CHECK_DIGIT_UI_SPEC.md`](../P3_BARCODE_CHECK_DIGIT_UI_SPEC.md). |
| Barcode P4 ratio / density / physical quiet zone | Read-only Properties references `18:69`/`13:2`; current WPF exposes logical quiet-zone modules and X-dimension but no ratio, derived density or physical-mm QZ state | P4 is a design/contract slice. See [`P4_BARCODE_RATIO_QUIET_ZONE_UI_HANDOFF.md`](../P4_BARCODE_RATIO_QUIET_ZONE_UI_HANDOFF.md) and [`P4_BARCODE_RATIO_QUIET_ZONE_UI_SPEC.md`](../P4_BARCODE_RATIO_QUIET_ZONE_UI_SPEC.md); approve first symbology, QZ convention and runtime evidence before changing the panel. |
| Barcode P5 2D parity | Read-only Properties reference `18:69`/`13:2`; current WPF exposes QR-named sizing/EC controls for both QR and Data Matrix, while the source has no first-class DM size/EC fields | P5 is a design/contract slice. See [`P5_2D_BARCODE_PARITY_UI_HANDOFF.md`](../P5_2D_BARCODE_PARITY_UI_HANDOFF.md) and [`P5_2D_BARCODE_PARITY_UI_SPEC.md`](../P5_2D_BARCODE_PARITY_UI_SPEC.md); confirm renderer-supported DM semantics and runtime evidence before changing the panel. |
| Barcode P6 GS1 AI diagnostics | Read-only Properties references `13:2`/`18:69`; current WPF exposes profile plus one combined validation string, while Core owns strict AI parsing, FNC1 normalization, registry provenance and preflight | P6 is a diagnostics-first design/contract slice. See [`P6_GS1_AI_UI_HANDOFF.md`](../P6_GS1_AI_UI_HANDOFF.md) and [`P6_GS1_AI_UI_SPEC.md`](../P6_GS1_AI_UI_SPEC.md); confirm demanded AI classes, segment presentation, registry update policy and runtime evidence before changing the panel. |
| Barcode P7 print method | Read-only shell recreation node `2:2` (`1440 x 900`) has Print & Output `2:39` with Setup/Preview/Print, but no method, capability or fallback state; current `PrintService` is graphic-only with no `PrintMethod`/native adapter contract | P7 is an ADR/pilot-gated Print & Output slice. See [`P7_PRINT_METHOD_UI_HANDOFF.md`](../P7_PRINT_METHOD_UI_HANDOFF.md) and [`P7_PRINT_METHOD_UI_SPEC.md`](../P7_PRINT_METHOD_UI_SPEC.md); keep Graphic as the default, expose requested/resolved path and explicit fallback, and do not claim native support until a printer-family pilot exists. |
| Barcode P8 physical verifier / grade | Current Core has hash-only `PhysicalOutputVerificationEvidence`, adapter timeout/busy guards and evidence-bound lifecycle completion; current Print Center has recovery/support-export controls but no verifier/device panel. Figma shell `2:2` and History `3:85` contain no verifier state. | P8 is a lab-first evidence UI contract. See [`P8_PHYSICAL_VERIFIER_UI_HANDOFF.md`](../P8_PHYSICAL_VERIFIER_UI_HANDOFF.md) and [`P8_PHYSICAL_VERIFIER_UI_SPEC.md`](../P8_PHYSICAL_VERIFIER_UI_SPEC.md); keep queue/preflight/golden/visual inspection separate from physical grade and wait for real fixture/adapter evidence. |
...
The new [`CC_P7_ADMINISTRATION_UI_SPEC.md`](../CC_P7_ADMINISTRATION_UI_SPEC.md) maps read-only Figma Administration `5:41` to local activation, preferences, data-source maintenance, evidence links and retention-preview boundaries. It does not authorize roles/users/sync/server-license features, destructive retention, runtime UI or a Figma edit.
...
The new [`P3_BARCODE_CHECK_DIGIT_UI_SPEC.md`](../P3_BARCODE_CHECK_DIGIT_UI_SPEC.md) turns the next open barcode phase into a reviewable Code 39-first policy/HRI contract: `None`/`Auto`/`Verify`, display-only HRI masking, fail-closed invalid states, proposed AutomationIds and responsive gates. It reuses Figma `18:69`/`13:2` only as interim read-only language; no barcode Properties implementation or Figma edit is implied.

**Superseding audit after the P3 barcode check-digit/HRI UI spec (2026-08-13):** `80` Markdown files, `575` relative links checked, and `0` broken paths. The `79`/`563` figures above are the preceding CC-P8 UI spec baseline; external URLs remain outside this local-path audit.

The new [`R4_DATA_WORKSPACE_UI_SPEC.md`](../R4_DATA_WORKSPACE_UI_SPEC.md) turns the v0.211 follow-up into a reviewable authoring/diagnostics contract: one derived field, sample value, deterministic lineage, atomic Apply and fail-closed parse/missing/duplicate/cycle states. It reuses Figma Data shell nodes `9:2`, `9:3`, `9:16`, `9:27` and `9:35` read-only; no transform editor, runtime implementation or Figma edit is implied.

**Superseding audit after the R4 Data Workspace UI spec (2026-08-13):** `81` Markdown files, `583` relative links checked, and `0` broken paths. The `80`/`575` figures above are the preceding P3 barcode UI spec baseline; external URLs remain outside this local-path audit.

The new [`DATABASE_MANAGER_UI_SPEC.md`](../DATABASE_MANAGER_UI_SPEC.md) turns the existing Manager handoff into a runtime-verifiable list/detail contract: source ownership, async Test/Preview evidence, usage-aware removal, guarded cleanup, proposed AutomationIds and target-scale click-through. It reuses Figma `8:2`/`9:2` shell language read-only; no Manager frame or Figma edit is implied.

**Superseding audit after the Database Manager UI spec (2026-08-13):** `82` Markdown files, `593` relative links checked, and `0` broken paths. The `81`/`583` figures above are the preceding R4 Data Workspace UI spec baseline; external URLs remain outside this local-path audit.

The new [`P4_BARCODE_RATIO_QUIET_ZONE_UI_HANDOFF.md`](../P4_BARCODE_RATIO_QUIET_ZONE_UI_HANDOFF.md) and [`P4_BARCODE_RATIO_QUIET_ZONE_UI_SPEC.md`](../P4_BARCODE_RATIO_QUIET_ZONE_UI_SPEC.md) route the next barcode phase through a Code 39-first ratio policy, read-only density presentation and physical quiet-zone measurement from shared effective X. They do not add model fields, change barcode rendering, edit Figma or claim P4 complete.

**Superseding audit after the P4 barcode ratio/quiet-zone UI docs (2026-08-13):** `84` Markdown files, `616` relative links checked, and `0` broken paths. The `82`/`593` figures above are the preceding Database Manager UI spec baseline; external URLs remain outside this local-path audit.

The new [`P5_2D_BARCODE_PARITY_UI_HANDOFF.md`](../P5_2D_BARCODE_PARITY_UI_HANDOFF.md) and [`P5_2D_BARCODE_PARITY_UI_SPEC.md`](../P5_2D_BARCODE_PARITY_UI_SPEC.md) route the next barcode phase through standard-aware QR/Data Matrix copy, renderer-supported DM sizing/EC semantics, shared fixed-module DPI risk and explicit unavailable states. They do not add model fields, change barcode rendering, edit Figma or claim P5 complete.

The P5 read-only Figma check confirmed selected Properties node `18:69` (`300 × 700`, `284 DIP` content cards); no dedicated barcode 2D state is present. Use the shell only for grouping/status language until the owner approves the standard-specific contract and target-scale runtime evidence.

**Superseding audit after the P5 2D barcode parity UI docs (2026-08-13):** `86` Markdown files, `642` relative links/assets checked, and `0` broken paths. The `84`/`616` figures above are the preceding P4 baseline; external URLs remain outside this local-path audit.
...
The new [`P6_GS1_AI_UI_HANDOFF.md`](../P6_GS1_AI_UI_HANDOFF.md) and [`P6_GS1_AI_UI_SPEC.md`](../P6_GS1_AI_UI_SPEC.md) route the next barcode phase through a diagnostics-first GS1 subset surface: strict notation, AI/boundary/FNC1 explanation, registry provenance and separate geometry/preflight status. They do not add a full AI wizard, change parser/rendering behavior, edit Figma or claim certification.

The P6 read-only Figma check confirmed selected Properties node `13:2` (`300 × 700`, `276 DIP` content card); no GS1-specific state is present. Use the shell only for grouping/status density until the owner approves the demanded AI classes, `[FNC1]` copy, registry update policy and target-scale runtime evidence.

**Superseding audit after the P6 GS1 AI UI docs (2026-08-13):** `88` Markdown files, `669` relative links/assets checked, and `0` broken paths. The `86`/`642` figures above are the preceding P5 baseline; external URLs remain outside this local-path audit.

The new [`P7_PRINT_METHOD_UI_HANDOFF.md`](../P7_PRINT_METHOD_UI_HANDOFF.md) and [`P7_PRINT_METHOD_UI_SPEC.md`](../P7_PRINT_METHOD_UI_SPEC.md) route the next barcode phase through an ADR-first Print & Output contract: Graphic remains the default, Native is explicit and pilot-gated, requested/resolved paths and fallback reasons are persisted, and no queue or method may change silently. They do not add a native command emitter, claim ZPL/EPL/TSPL support, alter barcode geometry or close P7.

The P7 read-only Figma check confirmed shell node `2:2` (`1440 x 900`) with Print & Output `2:39`, Setup `2:41`, Preview `2:44`, Print `2:47`, printer status `2:19` and paper status `2:21`. No Print Method, capability evidence, resolved path or fallback state exists in the shell; use it only for grouping and density until a printer-family pilot and runtime evidence are approved.

**Superseding audit after the P7 print-method UI docs (2026-08-13):** `90` Markdown files, `689` relative links/assets checked, and `0` broken paths. The `88`/`669` figures above are the preceding P6 baseline; external URLs remain outside this local-path audit.

The new [`P8_PHYSICAL_VERIFIER_UI_HANDOFF.md`](../P8_PHYSICAL_VERIFIER_UI_HANDOFF.md) and [`P8_PHYSICAL_VERIFIER_UI_SPEC.md`](../P8_PHYSICAL_VERIFIER_UI_SPEC.md) route the open hardware phase through a job-level, hash-only evidence surface: queue/preflight/thermal-golden states remain distinct from physical verification, visual inspection is audit-only, grade scale and threshold are explicit, and completion requires accepted evidence bound to the manifest. They do not add a device SDK, claim ISO/ANSI certification, add a verifier UI or close P8.

The P8 read-only Figma check confirmed shell node `2:2` (`1440 x 900`) with Print & Output `2:39` and status bar `2:170`, plus Control Center History node `3:85` (`1280 x 800`) with filters `3:99` and activity frame `3:101`. Neither has verifier/device/grade states; use them only for placement and row/detail hierarchy until a fixture and adapter supply concrete evidence.
...
**Superseding audit after the P7 print-method owner decision packet (2026-08-13):** `98` Markdown files, `845` relative links/assets checked, and `0` broken paths. The `97`/`816` figures above are the preceding P6 GS1 owner-packet baseline; external URLs remain outside this local-path audit.

**Superseding audit after the P8 physical-verifier owner decision packet (2026-08-13):** `99` Markdown files, `875` relative links/assets checked, and `0` broken paths. The `98`/`845` figures above are the preceding P7 print-method owner-packet baseline; external URLs remain outside this local-path audit.

**Superseding audit after the CC-P3 document-library/revision owner decision packet (2026-08-13):** `100` Markdown files, `899` relative links/assets checked, and `0` broken paths. The `99`/`875` figures above are the preceding P8 physical-verifier owner-packet baseline; external URLs remain outside this local-path audit.

**Superseding audit after the CC-P4 approval-workflow owner decision packet (2026-08-13):** `101` Markdown files, `923` relative links/assets checked, and `0` broken paths. The `100`/`899` figures above are the preceding CC-P3 document-library/revision owner-packet baseline; external URLs remain outside this local-path audit.
