...
**Still partial after this slice (explicit):**

- Graphic render still **fills the object frame** (bars distributed across width). Authored X drives **policy/preflight**, not yet auto object width = X × modules.  
- No continuous “effective mils/dots @ printer” readout chrome (warning only).  
- No mil unit field; mm only.
...
| **P1** | M2 partial, M5, M6 chrome | Physical geometry fidelity (auto-width, mils readout, bar height clarity) | P0 |
| **P2** | M9, M10–M12 subset | HRI placement & layout (None / Below / Above) — **DONE** | P0; prefer before heavy HRI chrome |
| **P3** | M7, M8 | Check-digit policy + HRI show/hide check digit — **DONE** | P2 recommended |
...
> **Detailed next-slice plan (2026-08-12 timed research):**  
> [`P1_LINEAR_GEOMETRY_NEXT_SLICE.md`](P1_LINEAR_GEOMETRY_NEXT_SLICE.md)  
> Research notes: session scratch `p1-next-plan-research-notes.md` (timer ≥30 min).
...
### P3 — Check-digit policy + HRI check-digit display — `[x]` DONE (2026-08-12)
...
1. **P1** — auto-width from X + effective mils readout.  
2. **P2** — HRI None/Below/Above.  
3. **P3** — check digit + HRI hide check digit.  
4. **P4** — ratio + quiet zone mm.  
5. **P5** — Data Matrix UI parity.  
6. **P6** — GS1 AI growth (incremental).  
7. **P7** — native print method (ADR-first).  
8. **P8** — hardware verifier (lab-first).
...
- [ ] Frame/width consistent with quantized X when operator opts in (P1).  
- [ ] HRI placement industrial triad None/Below/Above (P2).  
