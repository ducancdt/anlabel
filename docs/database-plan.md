...
**Ngày:** 2026-07-02 · **Trạng thái:** Đề xuất, chờ duyệt
...
1. **Lưu đường dẫn kép trong `DatabaseConfig`**: giữ `FilePath` tuyệt đối (tương thích ngược) + thêm `RelativePath` (tương đối so với vị trí file `.anlabel`). Khi mở template, thử theo thứ tự: tuyệt đối → tương đối → cùng thư mục với `.anlabel` → hỏi người dùng (dialog "Locate Excel file..." và lưu lại lựa chọn).
2. **Nút re-link rõ ràng**: khi báo "Excel link could not be restored", hiện luôn nút `Chọn lại file Excel...` thay vì chỉ statusText — người dùng sửa được ngay tại chỗ.
3. **Test**: `template excel link survives folder move` — save template + Excel cùng thư mục, copy cả thư mục sang chỗ khác, mở lại phải tự resolve bằng RelativePath.
3b. **Không ghi đè `sample-data.xlsx` vô điều kiện** (điểm yếu 7): chỉ giải nén khi file chưa tồn tại hoặc app version mới hơn lần giải nén trước (lưu marker version cạnh file). Người dùng sửa file mẫu không bị mất dữ liệu khi khởi động lại.
...
6b. **Cancel/timeout khi đọc file** (điểm yếu 8): `CancellationToken` cho `GetSheetNames`/`LoadSheetAsync`, nút Cancel trên UI, timeout mặc định (~30s) cho đường dẫn UNC/network với message riêng. Thử mở stream `FileShare.ReadWrite` trước khi đưa cho ClosedXML để đọc được cả khi file đang mở trong Excel.
