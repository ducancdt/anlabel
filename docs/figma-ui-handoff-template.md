...
Barcode UI coverage note: the same page has only the compact-ribbon text layer `1:8` (`Text TextBox Image Barcode`) for barcode authoring. That is a navigation hint, not a Properties/state design. P1/P2 barcode software slices are closed; P3 check-digit/HRI, P4 ratio/quiet-zone, P5 2D parity and P6 GS1 diagnostics are routed through their review artifacts. Any P3+ UI slice must provide a state-specific node (or an explicit decision to reuse the existing WPF surface), measured controls, and runtime evidence. Do not infer check-digit policy, HRI display, X-dimension, 2D size/EC semantics, GS1 AI coverage or preflight copy from that text layer alone.
