...
Barcode UI coverage note: the same page has only the compact-ribbon text layer `1:8` (`Text TextBox Image Barcode`) for barcode authoring. That is a navigation hint, not a Properties/state design. P1/P2 barcode software slices are closed; P3 check-digit/HRI, P4 ratio/quiet-zone, P5 2D parity, P6 GS1 diagnostics and P7 print-method review are routed through their review artifacts. Any P3+ UI slice must provide a state-specific node (or an explicit decision to reuse the existing WPF surface), measured controls, and runtime evidence. Do not infer check-digit policy, HRI display, X-dimension, 2D size/EC semantics, GS1 AI coverage, print-method resolution or preflight copy from that text layer alone.
...
| Database Manager | Needs runtime/design review | Panels Page `0:1` has no Manager frame; current WPF `DatabaseManagerWindow` already has list/detail, Test Connection, Preview, Use, Remove and Cleanup states. The reviewable runtime contract is [`DATABASE_MANAGER_UI_SPEC.md`](DATABASE_MANAGER_UI_SPEC.md). | Follow [`DATABASE_MANAGER_UI_HANDOFF.md`](DATABASE_MANAGER_UI_HANDOFF.md): approve current WPF information architecture or name a state-specific reference, then close the runtime state matrix. |
