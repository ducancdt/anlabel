...
P5 History owner note: use [`CC_P5_HISTORY_REPRINT_UI_DECISION_PACKET.md`](CC_P5_HISTORY_REPRINT_UI_DECISION_PACKET.md) for the concrete read-model/source-precedence, identity, time/privacy, diagnostics and History-to-Print-Center ownership boundary. It complements the recovery owner packet and does not authorize a History host, runtime merge or Figma write.
...
CC-P5 routing note: Control Center History metadata `3:85` supplies a `1248 x 56` filter bar and `1248 x 600` activity frame, with a note for details/reprint/errors but no concrete child states. Route the local three-source read model and exact-manifest reprint gate through [`CC_P5_HISTORY_REPRINT_UI_HANDOFF.md`](CC_P5_HISTORY_REPRINT_UI_HANDOFF.md) and its concrete owner packet [`CC_P5_HISTORY_REPRINT_UI_DECISION_PACKET.md`](CC_P5_HISTORY_REPRINT_UI_DECISION_PACKET.md); do not treat the Figma sample rows as live history or physical-output evidence.
...
| CC-P5 History + controlled reprint | Owner packet open; host/runtime read-model gate remains upstream | Control Center `3:85` gives filters/activity/detail affordances; current WPF has CSV history, job JSONL, hash-chained state/recovery and guarded reprint services, but no unified browser | Follow [`CC_P5_HISTORY_REPRINT_UI_HANDOFF.md`](CC_P5_HISTORY_REPRINT_UI_HANDOFF.md) and [`CC_P5_HISTORY_REPRINT_UI_DECISION_PACKET.md`](CC_P5_HISTORY_REPRINT_UI_DECISION_PACKET.md): approve provenance/retention/time/privacy semantics and keep Request → Approve → Prepare → Dispatch explicit. |
